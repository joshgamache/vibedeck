...
<html lang="en">
  <head>
    ...
    <style>
      :root {
        ...
        --glow: 0 0 20px rgba(212, 148, 58, 0.15);
        --radius: 12px;
      }
      * {
      ...
      .card-frame.flippable {
        cursor: pointer;
      }
      .card-frame.flippable:hover {
        transform: translateY(-4px);
        box-shadow:
          var(--shadow),
          0 0 30px rgba(212, 148, 58, 0.25);
      }
      .card-inner {
        width: 100%;
        height: 100%;
      ...
      .card-text-panel::-webkit-scrollbar {
        width: 4px;
      }
      .card-text-panel::-webkit-scrollbar-thumb {
      ...
      .btn-primary {
        ...
        padding: 16px 24px;
        font-size: 0.8rem;
        box-shadow: 0 4px 20px rgba(212, 148, 58, 0.25);
      }
      .btn-primary:hover {
        box-shadow: 0 4px 30px rgba(212, 148, 58, 0.45);
      ...
      .btn-blue {
        ...
        border: 1px solid rgba(64, 112, 184, 0.4);
        color: #7aabee;
      }
      .btn-blue:hover {
        background: rgba(64, 112, 184, 0.25);
        border-color: rgba(64, 112, 184, 0.7);
      }
      .history-header {
        display: flex;
        justify-content: space-between;
      ...
      .pair-ex-col.is-front .ex-label {
        background: rgba(160, 104, 32, 0.3);
        color: var(--amber);
      }
      .pair-ex-col.is-back .ex-label {
      ...
      .pt-front .pt-role-badge {
        background: rgba(160, 104, 32, 0.85);
        color: #ffe8b0;
      }
      .pt-back .pt-role-badge {
        background: rgba(40, 70, 130, 0.85);
        color: #aac8f0;
      }
      .pt-pair-num {
        position: absolute;
      ...
      .progress-bar-fill {
        height: 100%;
        background: linear-gradient(90deg, var(--amber-dim), var(--amber2));
        border-radius: 4px;
      ...
      .split-option-row input[type="radio"] {
      ...
      .card-frame.revealing {
        animation: cardReveal 0.35s cubic-bezier(0.34, 1.56, 0.64, 1);
      }
      .divider {
        height: 1px;
        background: var(--border);
  ...
  <body>
        ...
        <div id="view-draw" class="view active">
          ...
          <div class="deck-progress">
            <div class="deck-progress-bar">
              <div class="deck-progress-fill" id="deck-progress-fill" style="width:0%"></div>
            </div>
            <div class="deck-progress-text">
              <span id="progress-drawn">0 drawn</span><span id="progress-total">0 total</span>
            </div>
          </div>
          ...
          <div class="draw-controls">
            <div class="draw-controls-row">
              <button class="btn btn-primary" id="draw-btn" onclick="drawCard()" disabled>
                ✦ Draw Card
              </button>
            </div>
            <div class="draw-controls-row">
              ...
              <button class="btn btn-secondary" id="text-btn" onclick="toggleCardText()" disabled>
                ≡ Text
              </button>
            </div>
          </div>
          ...
          <div id="library-content">
            <div class="empty-state">
              ...
              <div class="empty-state-sub">
                Import a Monte Cook Games card deck PDF to get started.
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="modal">
        <div class="modal-header">
          <div class="modal-title">
            Import Deck<button class="modal-close" onclick="closeImportModal()">✕</button>
          </div>
          <div class="stepper">
          ...
          <div class="import-step active" id="istep-1">
            <div class="form-group">
              ...
              <input
                ...
                class="form-input"
                id="import-name"
                placeholder="e.g. Cypher Deck, Weird Deck…"
                oninput="checkStep1()"
              />
            </div>
            <div class="form-group">
            ...
            <div id="preview-loading" style="display:none">
              <div class="preview-loading">
                ...
                <div class="mini-progress">
                  <div class="mini-progress-fill" id="mini-pf" style="width:0%"></div>
                </div>
              </div>
            </div>
            <div id="preview-content" style="display:none">
              <div id="pair-example" class="pair-example" style="display:none">
                <div class="pair-ex-col is-front">
                  ...
                  <div class="ex-thumb"><img id="ex-front-img" src="" alt="" /></div>
                </div>
                <div class="pair-ex-arrow">⟷</div>
                <div class="pair-ex-col is-back">
                  <div class="ex-label">Back</div>
                  <div class="ex-thumb"><img id="ex-back-img" src="" alt="" /></div>
                </div>
                <div class="pair-ex-info">
                  <div class="pair-ex-title" id="pex-title"></div>
                  <div class="pair-ex-sub" id="pex-sub"></div>
                </div>
              </div>
          ...
          <div class="import-step" id="istep-3">
            <div class="import-progress-wrap">
              <div class="progress-bar-wrap">
                <div class="progress-bar-fill" id="imp-pf" style="width:0%"></div>
              </div>
        ...
        <div class="modal-footer">
          <div class="draw-controls-row" style="gap:10px">
            ...
            <button class="btn btn-secondary" id="ibtn-cancel" onclick="closeImportModal()">
              Cancel
            </button>
            <button class="btn btn-primary" id="ibtn-next" onclick="importNext()" disabled>
              Preview →
            </button>
          </div>
        </div>
      ...
      <div class="modal">
        <div class="modal-header">
          <div class="modal-title" style="font-size:.85rem">
            Split: <span id="split-deck-name" style="color:var(--text-dim);font-weight:400"></span
            ><button class="modal-close" onclick="closeSplitModal()">✕</button>
          </div>
        </div>
        <div class="modal-body">
          <div class="split-section-label">
            All Cards <span id="split-total" style="color:var(--text-muted)"></span>
          </div>
          ...
          <div class="split-ranges-section">
            <div class="split-section-label">
              Named Ranges<button
                class="btn btn-ghost"
                style="padding:5px 10px;font-size:.6rem"
          ...
          <div class="split-after-section">
            <div class="split-section-label" style="margin-bottom:8px">
              After creating sub-decks…
            </div>
            <label class="split-option-row"
              ><input type="radio" name="split-after" value="keep" checked />
              <div>
            ...
            >
            <label class="split-option-row"
              ><input type="radio" name="split-after" value="delete" />
              <div>
    ...
    <script>
      ...
      function iDelIdx(store, index, value) {
        return new Promise((res, rej) => {
      ...
      async function init() {
        ...
        ds.forEach((d) => (S.drawState[d.deckId] = d));
        const last = localStorage.getItem("lastDeckId");
        if (last && S.decks.find((d) => d.id === last)) S.currentDeckId = last;
        else if (S.decks.length) S.currentDeckId = S.decks[0].id;
        renderDeckSelect();
        renderLibrary();
        await loadDeckState();
        updateDrawView();
      ...
      async function onDeckChange(id) {
        if (!id) return;
        S.currentDeckId = id;
        S.currentCard = null;
        S.cardFlipped = false;
      ...
      async function loadDeckState() {
        ...
        if (!ds) {
          ...
          ds = { deckId: S.currentDeckId, remaining: shuffle(cards.map((c) => c.id)), drawn: [] };
      ...
      async function drawCard() {
        ...
        S.cardFlipped = false;
        S.showText = false;
        const entry = { deckId: S.currentDeckId, cardId: cid, drawnAt: Date.now() };
      ...
      function updateDrawView(animate = false) {
        ...
        const card = S.currentCard,
          hasBoth = card.front && card.back;
        front.innerHTML = `<img src="${card.front}" alt="" onclick="event.stopPropagation();openLightbox('${card.front}')">`;
        if (hasBoth) {
          back.innerHTML = `<img src="${card.back}" alt="" onclick="event.stopPropagation();openLightbox('${card.back}')">`;
        ...
        S.cardFlipped ? frame.classList.add("flipped") : frame.classList.remove("flipped");
        if (S.showText && card.text) {
          textPanel.textContent = card.text;
          textPanel.classList.add("visible");
        } else textPanel.classList.remove("visible");
      ...
      function handleCardClick() {
        ...
        S.cardFlipped
          ? document.getElementById("card-frame").classList.add("flipped")
          : document.getElementById("card-frame").classList.remove("flipped");
      }
      ...
      async function renderHistory() {
        ...
        if (!S.currentDeckId) {
          cont.innerHTML =
            '<div class="empty-state"><div class="empty-state-icon">🃏</div><div class="empty-state-title">No deck selected</div></div>';
          return;
        }
      ...
      function renderLibrary() {
        ...
        S.decks.forEach((deck) => {
          ...
          el.innerHTML = `<div class="deck-item-preview">${deck.previewImage ? `<img src="${deck.previewImage}" alt="">` : ""}</div><div class="deck-item-info"><div class="deck-item-name">${esc(deck.name)}</div><div class="deck-item-meta">${deck.cardCount} cards · ${fmtDate(deck.createdAt)}</div></div><div class="deck-item-actions"><button class="btn btn-ghost" onclick="selectDeckLib('${deck.id}')">Use</button><button class="btn btn-blue btn-ghost" onclick="openSplitModal('${deck.id}')">⌗ Split</button><button class="btn btn-danger btn-ghost" onclick="deleteDeck('${deck.id}')">✕</button></div>`;
          list.appendChild(el);
        });
        cont.innerHTML = "";
        cont.appendChild(list);
      }
      function selectDeckLib(id) {
        document.getElementById("deck-select").value = id;
        onDeckChange(id);
        document
          .querySelectorAll(".nav-tab")
          .forEach((t) => t.classList.toggle("active", t.textContent === "Draw"));
        document
          .querySelectorAll(".view")
          .forEach((v) => v.classList.toggle("active", v.id === "view-draw"));
      }
      ...
      function checkStep1() {
        document.getElementById("ibtn-next").disabled = !(
          impFile && document.getElementById("import-name").value.trim()
        );
      }
      function setLayout(val, el) {
        impLayout = val;
        document.querySelectorAll(".layout-option").forEach((l) => l.classList.remove("selected"));
      ...
      function onFileSelected(e) {
        ...
        if (!ni.value) ni.value = f.name.replace(/\.pdf$/i, "").replace(/[-_]/g, " ");
        checkStep1();
      }
      async function importNext() {
        ...
        try {
          ...
          for (let i = 1; i <= total; i++) {
            ...
            prevPages.push({ img: canvas.toDataURL("image/jpeg", 0.75), text, pageNum: i });
      ...
      function renderPrevGrid() {
        ...
        if (impLayout === "single") {
          ...
          document.getElementById("pex-title").textContent = isAF
            ? "Layout A: Odd=Front, Even=Back"
            : "Layout B: Odd=Back, Even=Front";
          document.getElementById("pex-sub").textContent = isAF
            ? "Page 1→Front, Page 2→Back, Page 3→Front…"
            : "Page 1→Back, Page 2→Front, Page 3→Back…";
      ...
      async function startImport() {
        ...
        try {
          ...
          for (let i = 1; i <= total; i++) {
            ...
            pages.push({ img: canvas.toDataURL("image/jpeg", 0.88), text, pageNum: i });
          ...
          const ds = { deckId, remaining: shuffle(cards.map((c) => c.id)), drawn: [] };
          await iPut("drawState", ds);
      ...
      async function openSplitModal(deckId) {
        ...
        grid.innerHTML =
          '<div class="empty-state" style="grid-column:1/-1;padding:20px 0"><div class="empty-state-icon" style="font-size:1.5rem">⏳</div><div class="empty-state-title">Loading…</div></div>';
      ...
      function renderSplitGrid() {
        ...
        splitCards.forEach((card) => {
          ...
          div.innerHTML = `<img src="${card.front}" alt="" loading="lazy"><div class="sc-bar" style="background:${r ? r.color : "transparent"}"></div><span class="sc-num">p${card.pageNum}</span>`;
          const src = card.front;
      ...
      function addSplitRange() {
        ...
        splitRanges.push({ id: "r" + Date.now(), name: "", start: "", end: "", color });
        renderSplitRanges();
      }
      ...
      function validateSplit() {
        const ok =
          ...
          splitRanges.every(
            (r) => r.name.trim() && parseInt(r.start) > 0 && parseInt(r.end) >= parseInt(r.start),
          );
      ...
      async function confirmSplit() {
        ...
        for (const r of splitRanges) {
          ...
          const nCards = rCards.map((c, i) => ({ ...c, id: `${nid}_${i}`, deckId: nid }));
          ...
          const ds = { deckId: nid, remaining: shuffle(nCards.map((c) => c.id)), drawn: [] };
          await iPut("drawState", ds);
          S.drawState[nid] = ds;
          S.decks.push(ndeck);
          S.history[nid] = [];
        ...
        if (after === "delete") {
          ...
          if (S.currentDeckId === splitDeckId) {
            S.currentDeckId = newDecks.length
              ? newDecks[0].id
              : S.decks.length
                ? S.decks[0].id
                : null;
        ...
        renderHistory();
        showToast(
          `Created ${newDecks.length} sub-deck${newDecks.length !== 1 ? "s" : ""}!`,
          "success",
        );
      }
